...
class BoardWidget extends Pad {
    ...
    /** Draw the crosshair on G at S. */
    private void drawCross(Graphics2D g, Square s) {
        g.setColor(Color.BLUE);
        int x = (int) (cx(s) + (0.5 * SQUARE_SIDE));
        int y = (int) (cy(s) + (0.5 * SQUARE_SIDE));
        g.drawLine(x + PIECE_OFFSET, y,
                x - PIECE_OFFSET, y);
        g.drawLine(x, y  + PIECE_OFFSET,
                x, y - PIECE_OFFSET);
    }

    /** Handle a mouse-button push on S. */
    private void mousePressed(Square s) {
        List<Move> legalMoves = _board.legalMoves(null);
        _legalmoves.clear();
        for (Move legal : legalMoves) {
            if (legal.getFrom() == s) {
                _legalmoves.add(legal);
            }
        }
        repaint();
    }
    ...
    /** List of legal moves.*/
    private List<Move> _legalmoves = new ArrayList<>();
